...
def pred_pic(model,model_name,img_path):
    """
    Predicts the segmentation mask for a given image using a pre-trained model and displays the result.

    Args:
        model (torch.nn.Module): The pre-trained model to be used for prediction.
        model_name (str): Path to the model's state dictionary (checkpoint).
        img_path (str): Path to the input image for prediction.

    Returns:
        None: Displays the segmentation mask overlaid on the input image.
    """
    ...
    test_image = pad_to_mod16(test_image)
    test_tensor = image_transforms(test_image).cuda()
    test_tensor = test_tensor.unsqueeze(0)
    test_output = (model(test_tensor))
    gs_output = test_output[0].argmax(dim=1)
    gs_output = (gs_output[0]).detach().cpu().numpy()
    
    #mask
    model_mask_color = coloring_masks(gs_output)
    # one on one
    seg_model = add_mask(test_image,model_mask_color)
    seg_model = seg_model.astype(np.uint8)  # Convert to uint8 type
    plt.imshow(seg_model)
...
def pred_examples(best_model_dir,model,example_list,test_dir):
    """
    Pads the input image so that its dimensions are divisible by 16.

    This is often required for deep learning models that work with fixed-size inputs.
    The padding is applied evenly at the bottom and right sides of the image.

    Args:
        arr (numpy.ndarray): Input image array to be padded. Expected to have shape (H, W, C), where H is the height, W is the width, and C is the number of channels.

    Returns:
        numpy.ndarray: The padded image with dimensions divisible by 16.
    """
    best_path = (glob.glob(os.path.join(best_model_dir, '*')))[0]
    for img in example_list:
        img_path = os.path.join(test_dir,img)
        pred = pred_pic(model,best_path,img_path)
        image = Image.fromarray(pred)
        output_path = os.path.join(best_model_dir, f'{img}_pred.png')
...
def geojson_to_rgb_image(geojson_path, img_size=(512, 512)):
    """
    Convert a GeoJSON file to an RGB image where polygons are colored using a custom colormap.
    
    Args:
        geojson_path (str): Path to the GeoJSON file.
        colormap (dict): Dictionary mapping some property (e.g., 'name') to RGB color tuples.
        img_size (tuple): Size of the output image (width, height).
        
    Returns:
        np.ndarray: RGB image with polygons drawn.
    """
    ...
    if gdf.empty:
        print("GeoDataFrame is empty. Check the GeoJSON file.")
        return np.zeros((img_size[1], img_size[0], 3), dtype=np.uint8)
    
    # Create a blank array with the correct size
    image_blank = np.zeros(shape = (img_size[1], img_size[0],3))
    lst = []
    for idx, row in gdf.iterrows():
        ...
        polygon_array = (np.array(geom.exterior.coords)) #list of coord of the poly
        lst.append((polygon_array,color))
    for item in lst:
        color = colormap[item[1]]
        if item[1] == 1:
            cv2.fillPoly(image_blank, np.int32([item[0]]), color)


            # Displaying the image
    cv2.imshow("Triangle",image_blank)
...
if __name__ == "__main__":
        # Example IoU scores for three models across different classes
    classes = ['Class 1', 'Class 2', 'Class 3']
    models = ['Model A', 'Model B', 'Model C']

    # Example IoU scores (replace with your actual results)
    model_a_iou = [0.2, 0.5, 0.8]
    model_b_iou = [0.3, 0.6, 0.9]
    model_c_iou = [0.4, 0.7, 0.95]

    # Create an array for each model's IoU scores
    iou_scores = np.array([model_a_iou, model_b_iou, model_c_iou])

    # Define bar width and index positions for the bars
    bar_width = 0.2
    index = np.arange(len(classes))

    # Plot the IoU scores
    plt.figure(figsize=(10, 6))
    plt.bar(index, model_a_iou, bar_width, label='Model A')
    plt.bar(index + bar_width, model_b_iou, bar_width, label='Model B')
    plt.bar(index + 2 * bar_width, model_c_iou, bar_width, label='Model C')

    # Add labels and titles
    plt.xlabel('Classes')
    plt.ylabel('IoU Score')
    plt.title('Benchmark Comparison of Segmentation Models')
    plt.xticks(index + bar_width, classes)
    plt.legend()
